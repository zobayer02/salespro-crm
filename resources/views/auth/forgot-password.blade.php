...
<head>
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        input { width: 100%; height: 62px; border: 1px solid rgba(148,163,184,.32); border-radius: 999px; padding: 0 22px; outline: none; color: #0f172a; background: rgba(255,255,255,.92); font-family: var(--font-secondary); font-size: 19px; font-weight: 700; }
...
<body>
    <main class="auth-card">
        ...
        <form method="POST" action="{{ route('password.email') }}">
            @csrf
            <input type="email" name="email" value="{{ old('email') }}" placeholder="Enter your email" autocomplete="email" required autofocus>
            @error('email')
